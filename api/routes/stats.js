...
2. Kategori tablosunda tekil veri sayısı.  aynı olanları teke düşürme
...
router.post("/auditlogs", async (req, res) => {
    try {
        ...
        let result = await AuditLogs.aggregate([  //aggregate: sql tarafında joine karşılık gelen yapı
            { $match: filter },
...
router.post("/categories/unique", async (req, res) => {
    try {
        ...
        let result = await Categories.distinct("name", filter);  //distinct: ilk parametrede vereceğimiz fielda göre tekil olan degerleri alır
